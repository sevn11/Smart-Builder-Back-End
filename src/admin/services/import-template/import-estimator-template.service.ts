...
@Injectable()
export class ImportEstimatorTemplateService {
    ...
    async processImport(element: GroupedImportData, templateId: number, user: User) {
        try {
            await this.databaseService.$transaction(async (tx) => {
                ...
                const updateTemplate = await tx.masterProjectEstimatorTemplate.update({
                    where: { id: templateId, isDeleted: false },
                    data: { profitCalculationType: company.profitCalculationType || ProfitCalculationType.MARGIN }
                })
                const header = await tx.masterProjectEstimatorTemplateHeader.create({
                    data: {
                        name: element.header,
                        mpetId: templateId,
                ...
                if (header && header.id) {
                    ...
                    await Promise.all(
                        element.items.map(async (val) => {
                            ...
                            await tx.masterProjectEstimatorTemplateData.create({
                                data: {
                                    item: val.item,
    ...
    async checkTemplateExist(type: string, body: { templateId: string }) {
        try {
            const projectEst = await this.databaseService.masterProjectEstimatorTemplate.findUnique({
                where: {
